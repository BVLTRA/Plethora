...
export default function ReadStory() {
  ...
  const handleLike = async () => {
    if (!user) {
      showAlert("You must be connected to acknowledge a signal.");
      return;
    }
    ...
    try {
      if (!isLiked) {
        const response = await fetch(
          "http://localhost/plethora_api/likes.php",
          {
            method: "POST",
  ...
  const executeDeleteSequence = () => {
    // Wrap deletion logic inside custom confirmation thing... im so tired
    showConfirm(
      "Are you sure you want to permanently erase this signal?",
      async () => {
        try {
          const response = await fetch(
            "http://localhost/plethora_api/delete_entry.php",
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
          ...
          if (response.ok) {
            showAlert("Signal erased from the grid.", () =>
              navigate("/discover"),
            );
          } else {
            const data = await response.json();
            showAlert(data.error || "Failed to erase signal.");
          }
        } catch (error) {
  ...
  if (isLoading)
    return (
      ...
      >
        <div style={{ color: "#596f62", fontFamily: "Courier New" }}>
          Decrypting signal...
        </div>
      </main>
    );
  if (error || !story)
    return (
      ...
      >
        <div style={{ color: "#dc2626", fontFamily: "Courier New" }}>
          {error}
        </div>
      </main>
    );
        ...
        <article className="original-story">
          ...
          <div className="interaction-bar">
            <div className="interaction-buttons">
              ...
              <button
                className="circular-action-btn comment-btn"
